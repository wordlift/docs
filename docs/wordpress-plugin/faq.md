...
Review the [configuration settings](getting-started.html#configuration) for more information.
:::
...
5. Your vocabulary (article metadata and entities) are published as [linked data](http://docs.wordlift.it/en/latest/key-concepts.html#linked-open-data) and you can always request a data dump in any of the following formats: RDF/XML, Turtle, N3, JSON-LD.
...
Review the [Edit Entity page](edit-entity.html#entity-types-and-properties-table) for more information.
...
- Every entity should link to other entities - when we select other entities to enrich the description of an entity we create relationships in the site's [knowledge graph](key-concepts.html#knowledge-graph).
...
WordLift by default uses DBpedia and Freebase to detect and link named entities. With a custom configuration, the content analysis services provided by [Redlink](http://www.redlink.co) and available via our professional services, can use any RDF-based [graph](key-concepts.html#knowledge-graph). It is also possible to use *multiple graphs* for named entity recognition and [dereferencing](key-concepts.html#dereferencing-http-uris).
...
A triple is a set of three elements: a subject, a predicate, and an object. Triples are linked together to form a [graph](key-concepts.html#knowledge-graph) that is without hierarchy, is machine readable, and can be used to infer new facts. Triples in WordLift describe facts as metadata about an article or an entity.

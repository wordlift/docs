...
- `--site` (required unless `gsc_site_id` is set; `gsc.id` remains legacy fallback): GSC property URL (e.g. `sc-domain:example.com`).
...
- `--client-secrets`: OAuth client secrets JSON for GSC (also used for GA4).
...
- `--ga-id`: GA4 property ID to enable the Analytics section.
  - profile fallback supports `ga_property_id` (preferred) and `ga.id` (legacy), plus env `GA_ID`.

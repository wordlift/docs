...
| `schema_type` | string | Schema.org type to generate (e.g., `Review`). Optional if using `--type` or auto inference. |

## Options

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `--type` | string | none | Schema.org type to generate (e.g., `Review`). If omitted, the agent infers the type. |
...
| `--debug` | bool | `false` | Write agent prompt/response to `.structured-data/agent_debug.json`. |
